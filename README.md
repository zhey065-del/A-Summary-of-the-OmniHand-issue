...
通过键盘wsed四个按键实现核心关节运动

1. 编写键盘遥控节点脚本
请打开你的 Ubuntu 终端，进入之前建好的 Python 功能包目录：

cd ~/omnihand_pro_ws/src/hand_controller/hand_controller

新建一个名为 keyboard_teleop.py 的文件并打开它：

gedit keyboard_teleop.py
...
import termios
import tty
# 这里已经为你补全了完整的 12 个独立关节，彻底解决 RViz TF 树断裂不更新的问题
JOINTS = [
...
class KeyboardTeleop(Node):
    def __init__(self):
        ...
        self.joint_angles = {joint: 0.0 for joint in JOINTS}
        self.step = 0.05
        self.get_logger().info('🤖 12自由度键盘控制节点已启动！')
        self.get_logger().info('操作说明：')
        self.get_logger().info('W / S : 控制食指弯曲/伸直')
        self.get_logger().info('E / D : 控制中指弯曲/伸直')
        self.get_logger().info('按 Ctrl+C 退出')
    def publish_joints(self):
        msg = JointState()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.name = list(self.joint_angles.keys())
        msg.position = list(self.joint_angles.values())
...
def main():
    ...
    try:
        while rclpy.ok():
            ...
            if key == 'w':
                node.joint_angles['R_index_mcp_joint'] += node.step
            elif key == 's':
                node.joint_angles['R_index_mcp_joint'] -= node.step
            elif key == 'e':
                node.joint_angles['R_middle_mcp_joint'] += node.step
            elif key == 'd':
                node.joint_angles['R_middle_mcp_joint'] -= node.step
            elif key == '\x03': # 捕捉 Ctrl+C 退出信号
                break
...
下一步：重新编译让代码生效
回到终端，必须执行编译指令（因为你修改了 Python 文件里的内容）：
cd ~/omnihand_pro_ws
...
关闭官方调试界面joint state publisher

source ~/omnihand_pro_ws/install/setup.bash

ros2 run hand_controller keyboard_teleop
